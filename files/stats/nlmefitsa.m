...
% Initialize the following variables so they will not appear to be functions
structural_model = [];
mean_phi = [];
...
%     Ai=Ai(:,ind_covariates);
    Ai = A(:,:,Aidx(i));
    DFAi= DFi*Ai;
    Gi=DFi*Omega*DFi' + diag(g0i.^2);  %variance of zi, Gi is symmetric
    %
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % calculate log(det(Gi)) protecting against overflow
    % D's diagonal are the  previously calculated eigenvalues of Gi
    
        if any(diag(D)<0)
            warning(message('stats:nlmefitsa:PosdefLikelihood'));
            ll_lin = NaN;
        end
        
        logdet = sum(log(diag(D)));
    
%     % Useful for debugging
%    [~, U, P] = lu(Gi); % lu will still "work" in the case that Gi is not positive definite, which is not desirable
%     assert(mod(sum([diag(U); det(P)] < 0),2) == 0) % negatives occur in pairs
%     assert(~any(diag(U)==0)) % no zeros
%     logdet = sum(log(abs(diag(U))));
    
    ll_lin = ll_lin - 0.5*logdet - 0.5*Dzi'*IGi*Dzi ;
 end
...
if (~isempty(ind_fixed_est))
    if rcond(-Fmu)<eps(Fmu)
        warning(message('stats:nlmefitsa:FisherInformationSingular'))    
    end
    Cth=inv(-Fmu);
else
    Cth=[];
