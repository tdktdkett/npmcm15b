% GPPDF 一般化パレート確率密度関数 (pdf)
%
%   Y = GPPDF(X,K,SIGMA,THETA) は、X の値で評価される、すその指数 (形状) 
%   パラメータ K、スケールパラメータ SIGMA、閾値 (位置) パラメータ THETA 
%   をもつ一般化パレート (GP) 分布の pdf を返します。Y のサイズは、入力
%   引数の共通のサイズです。スカラの入力は、他の入力と同じサイズの定数
%   行列として機能します。
%
...
%
%   参考 GPCDF, GPFIT, GPINV, GPLIKE, GPRND, GPSTAT, PDF.
